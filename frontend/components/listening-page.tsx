...
import { StoryPipeline, getStoryInfo, getStoryPageCount } from "@/components/story-pipeline";
import { useLanguage } from "@/components/LanguageProvider";
import { useState } from "react";

export default function StoryReadingPage({ params }: { params: { slug: string } }) {
  ...
  // Get just the text
  const pageText = <StoryPipeline storyId={storyId} pageIndex={currentPage} contentType="text" />;
  
  // Get just the image URL
  const imageUrl = <StoryPipeline storyId={storyId} pageIndex={currentPage} contentType="image" />;
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="story-content-wrapper">
        ...
        {audioUrl && (
          <audio controls src={String(audioUrl)} className="mt-4 w-full"></audio>
        )}
      </div>
