		...
		.map((story) => ({
			...
			activities: story.activities.filter(
				(activity) =>
					!selectedLanguage ||
					!activity.languages ||
					activity.languages.includes(selectedLanguage)
			),
		}))
					...
					{filteredStories.length === 0 && (
						<div className="col-span-full flex flex-col items-center justify-center py-12 text-center">
							...
							<p className="text-muted-foreground mt-1">
								No {activityLabel} activities are available in{" "}
								{selectedLanguage}
							</p>
							<Button
								variant="outline"
										...
										{stories.map((story) =>
											story.activities.map((activity) =>
												selectedActivities[activity.id] ? (
													<div
														key={activity.id}
														className="flex items-center justify-between border rounded-md p-2 bg-muted/10"
													>
